from flask import Flask, request, abort, jsonify, make_response, render_template, send_from_directory
...
def login():
    ...
    if user is not None:
        ...
        return jsonify(message='Successfully logged in', token=access_token.encode('utf-8')), 200
    else:
...
def getProductDetails(product_id):
    cursor = mysql.cursor()
    cursor.execute("SELECT product_id, product_name, product_image, product_price, product_description FROM products WHERE product_id='"+product_id+"'") # SQL INJECTION
